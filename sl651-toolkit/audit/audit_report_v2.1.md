...
| 报文尾部多余字节 / 二进制路径结束符不校验 | **试行硬校验后回退**：福建规定样本文件本身有 9 条帧尾部粘着下一帧起始字节 0x7E，且「无效 BCD 优雅降级」测试帧结束符为 0xFF——真实抓包数据与此工具「宽容解析、CRC 标记」的设计哲学冲突，保持宽容 |
| ASCII 多值标注 `[1]` 起 | 装饰性 |

### 补充修复（R1~R5，2026-08-26 第二轮）

| # | 位置 | 修复 |
|---|------|------|
| R1 | `sl427/decoder.py:_parse_comprehensive` | array 型要素全 0xFF/0xAA 填充时按定义 byteLen 推进偏移，后续要素不再错位 |
| R2 | `sl651/decoder.py:decode()` | 入口补起始符校验（7E7E / 01），垃圾 bytes 直接 `DecodeError` |
| R3 | `sl651/decoder.py` 尾部垃圾/结束符硬校验 | **试行后回退**（见保留项），保持宽容解析 |
| R4 | `sl427/decoder.py:_parse_signed_bcd` | 高半字节 0xA~0xE 降级为 `-`；非法 BCD 不再抛错 |
| R5 | `sl427/decoder.py:_fmt_time_427` | month/day/时分秒范围校验，非法显示「无效时间(raw)」 |
...
- `tests/test_sl651.py`：25 → **41 项**，全部通过（新增 12 项回归：H1~H4、Hex 型 FF、两侧编码器校验、FFH Tp 剥离、Web API、CLI 脱敏、YAML 校验、温度有界、边沿触发）
