...
> 最后更新：2026-07-01  
> 版本：v1.2.2  
> 接手前必读：`docs/requirements.md` + 项目 `README.md` + 本文档
...
| `docs/requirements.md` | 需求基线：协议规格、功能码矩阵、要素编码、测试清单、Roadmap | 需求变更 / 功能补全 / 测试增减时同步更新 |
...
`sl651/__init__.py:__version__ = "1.2.2"`

### 1.3 完成的审计轮次

v1.1~v1.8，共 8 轮，全部缺陷已闭环。
...
| 任务 | 位置 | 说明 |
|------|------|------|
| `CTRL_FUNC_MAP[0x0D]` 标签 | `sl427/constants.py:96` | 已加 TODO 注释。0x0D 在不同上下文含义不同（下行=报警状态, 上行=统计雨量, AFN=84H=电压），需要明确 |
...
| `COMP_BITS` 气象映射 | D3 位仅映射风速(0x08)，未含气压(0x07)，不影响解析 |

### 3.3 设计保留项（不修）

| 项 | 位置 | 原因 |
|----|------|------|
| `AFN_NO_AUX` / `AFN_TP_ONLY` / `AFN_PW_TP` | `sl427/constants.py:130` | 文档用途，编解码器在调用处硬编码 AUX 逻辑 |
...
1. **`docs/requirements.md`** — 协议需求、功能码矩阵、测试清单、Roadmap
