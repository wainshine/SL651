...
python tests/run_all.py  → 8/8 套件通过
  主套件 60 项 / 盲区 10 项 / 模拟器集成 5 组 / 解码器变异 4 组
...
- M-1: `sl427/decoder.py` 缺测填充处理补全至状态/事件/水泵/中继/水质等全分支，`0xAA`/`0xFF` 统一降级 `-`
...
| **SL427 缺测填充统一降级** | 规约 `0xAA`/`0xFF` 为「未采集/无数据」占位（规约行 1172/900）：`_is_fill` 判定后，BCD 值/状态位图/事件计数/水泵数据/中继状态/水质等**全分支**统一降级 `-`，不拒帧也不输出误导值；非填充非法 BCD 仍抛 `DecodeError`（审计 v2.4 M-1/M-2/L-3） |
...
| `tests/test_sl651.py` | 60 项测试：BCD/CRC/定义符/编解码往返/福建23条+北京25条（CRC + 要素级基线）/0x31均匀报/F5固定长度/模拟器冒烟/编码器校验/Web API/功能码名称一致性/人工置数 F2/缺测填充全分支 等 |
