...
python tests/run_all.py  → 8/8 套件通过
  主套件 57 项 / 盲区 10 项 / 模拟器集成 5 组 / 解码器变异 3 组
  / SL427 参数 AFN 11 组 / SL427 查询 AFN 20 组 / SL427 控制 AFN 9 组 / SL651 多包重组 5 组
...
`sl651/__init__.py:__version__ = "1.3.0"`

### 1.3 完成的审计轮次

v1.1~v2.2，共 12 轮。v1.2.4 修复 v1.10/v2.0 综合参数/小时报F4/统计雨量等；v1.2.5 修复 v2.1 异常契约/线程安全/编码器校验；v1.2.6 修复 3 项自审计缺陷；v1.2.7 修复 v2.2 的 1C+5M+7L 全部缺陷；v1.2.8 工程化 + 测试深度 + SL427 参数 AFN；v1.3.0 SL427 全 AFN + SL651 编码器补全 + 多包重组。

### 1.4 v1.3.0 SL427 全 AFN 与 SL651 编码器补全（主会话 5 代，2026-09-14）
...
### 1.5 v1.2.8 工程化与功能扩展（主会话 5 代，2026-09-14）
...
### 1.6 v1.2.7 审计 v2.2 修复（主会话 5 代，2026-09-14）
...
### 1.7 v1.2.6 自审计修复（主会话 4 代，2026-09-03）
...
| **0x26 累计雨量为不归零累计** | `RainGenerator.total_accum`，上限 99999.9mm（N(6,1)）后翻转；日/小时累计仍按周期归零 |
...
| `tests/test_sl651.py` | 57 项测试：BCD/CRC/定义符/编解码往返/福建23条+北京25条（CRC + 要素级基线）/0x31均匀报/F5固定长度/模拟器冒烟/编码器校验/Web API 等 |
