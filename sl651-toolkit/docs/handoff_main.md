...
python tests/run_all.py  → 8/8 套件通过
  主套件 61 项 / 盲区 10 项 / 模拟器集成 5 组 / 解码器变异 4 组
...
`sl651/__init__.py:__version__ = "1.3.2"`

### 1.3 完成的审计轮次

v1.1~v2.4，共 14 轮。v1.2.4 修复 v1.10/v2.0 综合参数/小时报F4/统计雨量等；v1.2.5 修复 v2.1 异常契约/线程安全/编码器校验；v1.2.6 修复 3 项自审计缺陷；v1.2.7 修复 v2.2 的 1C+5M+7L 全部缺陷；v1.2.8 工程化 + 测试深度 + SL427 参数 AFN；v1.3.0 SL427 全 AFN + SL651 编码器补全 + 多包重组；v1.3.1 修复 v2.3 的 4M+6L；v1.3.2 修复 v2.4 的 2M+5L。

### 1.4 v1.3.2 审计 v2.4 修复（主会话 6 代，2026-09-14）

- M-1: `sl427/decoder.py` 缺测填充处理补全至**全部分支**——查询响应（状态/事件/水泵/中继/水质/种类/时钟）、控制/配置响应（90/91/92/93/94/95/A0/A1）、AFN=51，`0xAA`/`0xFF` 统一降级 `-`
- M-1b: `sl651/decoder.py` 45H 状态位全 `0xFF`/`0xAA` 填充降级 `-`
- M-2: AFN=84 自报电压帧遇填充改用 `_safe_bcd_le`，降级 `-` 并写 `warnings`
- L-1: README 辅助套件计数同步为实测 13/21/10/6
- L-2: 北京水务报文测站数更正为 10（README/project.md/handoff/示例注释）
- L-3: `_safe_bcd_le` 仅容忍规约合法填充，非填充非法 BCD 仍抛 `DecodeError`
- L-4: 变异测试新增「合法帧/合法填充不得抛异常、不得产生错误值」正向断言
- L-5: `project.md §2.2` 功能码表补齐 0x36/0x38/0x39/0x3A/0x4B~0x4F
- 测试: 主套件 59 → 61 项；变异套件 3 → 4 组

### 1.5 v1.3.1 审计 v2.3 修复（主会话 6 代，2026-09-14）
...
### 1.6 v1.3.0 SL427 全 AFN 与 SL651 编码器补全（主会话 5 代，2026-09-14）
...
### 1.7 v1.2.8 工程化与功能扩展（主会话 5 代，2026-09-14）
...
### 1.8 v1.2.7 审计 v2.2 修复（主会话 5 代，2026-09-14）
...
### 1.9 v1.2.6 自审计修复（主会话 4 代，2026-09-03）
...
| **SL427/SL651 缺测填充统一降级** | 规约 `0xAA`/`0xFF` 为「未采集/无数据」占位（SL427 规约行 1172/900）：SL427 `_is_fill` 判定后，查询响应（状态/事件/水泵/中继/水质/种类/时钟）与**控制/配置响应（90~96/A0/A1）**的 BCD 值/位图/计数等**全分支**统一降级 `-`；SL651 45H 状态位全 `FF`/`AA` 同样降级 `-`。不拒帧、不输出误导值；非填充非法 BCD 仍抛 `DecodeError`（审计 v2.4 M-1/M-2/L-3） |
...
| `tests/test_sl651.py` | 61 项测试：BCD/CRC/定义符/编解码往返/福建23条+北京25条（CRC + 要素级基线）/0x31均匀报/F5固定长度/模拟器冒烟/编码器校验/Web API/功能码名称一致性/人工置数 F2/缺测填充全分支/SL651 45H 填充 等 |
...
| `examples/beijing_messages.txt` | 北京水务平台 25 条 (10测站, 3类报文) |
